...
def enc_test():
    ...
    #Negative tests#
    assert enc[0]["fullUrl"] != N_ENCOUNTER_FULL_URL, "negative test case failed"
    assert enc[0]["request"]["ifNoneExist"] != f"identifier=https://pediatrix.com/fhir/NamingSystem/patient-demographics-update-id|{MRN}-{FacilityId}-{DISCHARGE_DATETIME}", "negative test case failed"
    assert enc[0]["request"]["url"] != N_URL[2], "negative test case failed"
    assert enc[0]["request"]["method"] != N_REQUEST_TYPE[1], "negative test case failed"
    assert enc[0]["resource"]["account"][0]["reference"] != N_ACCOUNT_FULL_URL, "negative test case failed"
    assert enc[0]["resource"]["account"][0]["type"] != N_REFERENCE_TYPE[3], "negative test case failed"
    assert enc[0]["resource"]["class"]["code"] != N_CLASS_CODE, "negative test case failed"
    assert enc[0]["resource"]["class"]["display"] != N_CLASS_DISPLAY, "negative test case failed"
    assert enc[0]["resource"]["class"]["system"] != N_CLASS_SYSTEM, "negative test case failed"
    assert enc[0]["resource"]["identifier"][0]["system"] != N_IDENTIFIER_SYSTEM[0], "negative test case failed"
    assert enc[0]["resource"]["identifier"][0]["value"] != f"{MRN}-{FacilityId}-{admitdate}-12345", "negative test case failed"
    assert enc[0]["resource"]["identifier"][1]["system"] != N_IDENTIFIER_SYSTEM[1], "negative test case failed"
    assert enc[0]["resource"]["identifier"][1]["value"] != N_PACID, "negative test case failed"
    assert enc[0]["resource"]["participant"][0]["individual"]["reference"] != N_ATTENDING_PRACTIONER_FULL_URL, "negative test case failed"
    assert enc[0]["resource"]["participant"][0]["individual"]["type"] != N_REFERENCE_TYPE[4], "negative test case failed"
    assert enc[0]["resource"]["participant"][1]["individual"]["reference"] != N_REFERRING_PRACTIONER_FULL_URL, "negative test case failed"
    assert enc[0]["resource"]["participant"][1]["individual"]["type"] != N_REFERENCE_TYPE[4], "negative test case failed"
    assert enc[0]["resource"]["participant"][0]["type"][0]["coding"][0]["code"] != N_PRACTITIONER_CODE[0], "negative test case failed"
    assert enc[0]["resource"]["participant"][0]["type"][0]["coding"][0]["display"] != N_PRACTITIONER_DISPLAY[0], "negative test case failed"
    assert enc[0]["resource"]["participant"][0]["type"][0]["coding"][0]["system"] != N_PRACTITIONER_SYSTEM, "negative test case failed"
    assert enc[0]["resource"]["participant"][1]["type"][0]["coding"][0]["code"] != N_PRACTITIONER_CODE[1], "negative test case failed"
    assert enc[0]["resource"]["participant"][1]["type"][0]["coding"][0]["display"] != N_PRACTITIONER_DISPLAY[1], "negative test case failed"
    assert enc[0]["resource"]["participant"][1]["type"][0]["coding"][0]["system"] != N_PRACTITIONER_SYSTEM, "negative test case failed"
    assert enc[0]["resource"]["period"]["start"] != N_ADMIT_DATETIME, "negative test case failed"
    assert enc[0]["resource"]["period"]["end"] != N_DISCHARGE_DATETIME, "negative test case failed"
    assert enc[0]["resource"]["resourceType"] != N_URL[2], "negative test case failed"
    assert enc[0]["resource"]["serviceProvider"]["reference"] != N_ORGANIZATION_DEPARTMENT_FULL_URL, "negative test case failed"
    assert enc[0]["resource"]["status"] != N_ENCOUNTER_STATUS, "negative test case failed"
    assert enc[0]["resource"]["subject"]["reference"] != N_PATIENT_FULL_URL, "negative test case failed"
    assert enc[0]["resource"]["type"][0]["text"] != (N_ADMIT_TYPE+"|"+N_PATIENT_TYPE+"|"+N_FINANCIAL_CLASS)+"12345", "negative test case failed"
