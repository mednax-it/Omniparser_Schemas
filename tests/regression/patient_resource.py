...
def patient_test():
    ...
    #Negative test cases
    assert pat[0]["resource"]["identifier"][0]["value"] != N_MRN, "negative test case failed"
    assert pat[0]["resource"]["name"][0]["family"] != N_LASTNAME, "negative test case failed"
    assert pat[0]["resource"]["name"][0]["given"][0] != N_FIRSTNAME, "negative test case failed"
    assert pat[0]["resource"]["name"][0]["given"][1] != N_MIDDLENAME, "negative test case failed"
    assert pat[0]["resource"]["birthDate"] != N_DOB, "negative test case failed"
    assert pat[0]["resource"]["gender"] != N_GENDER, "negative test case failedr"
    assert pat[0]["resource"]["address"][0]["line"][0] != N_ADDRESS1, "negative test case failed"
    assert pat[0]["resource"]["address"][0]["line"][1] != N_ADDRESS2, "negative test case failed"
    assert pat[0]["resource"]["address"][0]["city"] != N_CITY, "negative test case failed"
    assert pat[0]["resource"]["address"][0]["state"] != N_STATE, "negative test case failed"
    assert pat[0]["resource"]["address"][0]["postalCode"] != N_ZIP, "negative test case failed"
    assert pat[0]["resource"]["address"][0]["country"] != N_COUNTRY, "negative test case failed"
    assert pat[0]["resource"]["address"][0]["type"] != N_ADDRESSTYPE, "negative test case failed"
    assert pat[0]["resource"]["telecom"][0]["value"] != N_HOMEPHONE, "negative test case failed"
    assert pat[0]["resource"]["telecom"][0]["use"] != N_HOMEPHONETYPE, "negative test case failed"
    assert pat[0]["resource"]["telecom"][2]["value"] != N_HOMEEMAIL, "negative test case failed"
    assert pat[0]["resource"]["telecom"][1]["value"] != N_WORKPHONE, "negative test case failed"
    assert pat[0]["resource"]["telecom"][3]["use"] != N_WORKPHONETYPE, "negative test case failed"
    assert pat[0]["resource"]["telecom"][3]["value"] != N_WORKEMAIL, "negative test case failed"
    assert pat[0]["resource"]["maritalStatus"]["coding"][0]["display"] != N_MARITALSTATUS, "negative test case failed"
    assert pat[0]["resource"]["identifier"][2]["value"] != N_HOSPITALPATIENTNUMBER, "negative test case failed"
    assert pat[0]["resource"]["identifier"][1]["value"] != N_SSN, "negative test case failed"
    assert pat[0]["resource"]["multipleBirthInteger"] != N_BIRTHORDER, "negative test case failed"

    assert pat[0]["resource"]["identifier"][0]["system"] != N_IDENTIFIER_SYSTEM, "negative test case failed"
    assert pat[0]["resource"]["identifier"][0]["type"]["coding"][0]["code"] != N_MR_CODE, "negative test case failed"
    assert pat[0]["resource"]["identifier"][0]["type"]["coding"][0]["display"] != N_MR_DISPLAY, "negative test case failed"
    assert pat[0]["resource"]["identifier"][0]["type"]["coding"][0]["system"] != N_MR_SYSTEM, "negative test case failed"
    assert pat[0]["resource"]["identifier"][1]["system"] != N_SSN_SYSTEM, "negative test case failed"
    assert pat[0]["resource"]["identifier"][1]["type"]["coding"][0]["code"] != N_SSN_CODE, "negative test case failed"
    assert pat[0]["resource"]["identifier"][1]["type"]["coding"][0]["display"] != N_SSN_DISPLAY, "negative test case failed"
    assert pat[0]["resource"]["identifier"][1]["type"]["coding"][0]["system"] != N_SSN_TERMINOLOGY_SYSTEM, "negative test case failed"
    assert pat[0]["resource"]["identifier"][2]["system"] != N_HOSPITAL_PATIENT_SYSTEM, "negative test case failed"
    assert pat[0]["resource"]["identifier"][2]["type"]["coding"][0]["code"] != N_PATIENT_INTERNAL_CODE, "negative test case failed"
    assert pat[0]["resource"]["identifier"][2]["type"]["coding"][0]["display"] != N_PATIENT_INTERNAL_DISPLAY, "negative test case failed"
    assert pat[0]["resource"]["identifier"][2]["type"]["coding"][0]["system"] != N_PATIENT_INTERNAL_SYSTEM, "negative test case failed"
    assert pat[0]["resource"]["maritalStatus"]["coding"][0]["code"] != N_MARITALSTATUS_CODE, "negative test case failed"
    assert pat[0]["resource"]["maritalStatus"]["coding"][0]["display"] != N_MARITALSTATUS_DISPLAY, "negative test case failed"
    assert pat[0]["resource"]["maritalStatus"]["coding"][0]["system"] != N_MARITALSTATUS_SYSTEM, "negative test case failed"
    assert pat[0]["fullUrl"] != N_PATIENT_FULL_URL, "negative test case failed"
    assert pat[0]["request"]["method"] != N_REQUEST_TYPE[0], "negative test case failed"

    assert pat[0]["request"]["url"] != f"Patient?identifier=https://pediatrix.com/fhir/NamingSystem/mrn-id|{N_MRN}&mrn-assigner={N_ORGANIZATION_FULL_URL}"

    assert pat[0]["resource"]["identifier"][0]["assigner"]["reference"] != N_ORGANIZATION_FULL_URL, "negative test case failed"
    assert pat[0]["resource"]["identifier"][2]["assigner"]["reference"] != N_ORGANIZATION_FULL_URL, "negative test case failed"
