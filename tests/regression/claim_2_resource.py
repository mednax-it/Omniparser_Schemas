...
def claim_2_test(resource_name, testfile, identifier_url, identifier_id):
    if(testfile == "1"):
        ...
        assert claim[1]["request"]["url"] == f'{RESOURCE_NAME[1]}?identifier={IDENTIFIER_URL}/{IDENTIFIER_ID[1]}|{MRN}-{FACILITY_ID}-{RELATEDPERSON_INSURANCE_ID[1]}', "did not match request url"
        ...
        assert claim[1]["resource"]["identifier"][0]["value"] == f'{MRN}-{FACILITY_ID}-{RELATEDPERSON_INSURANCE_ID[1]}', "did not match claim value"
