...
def claim_1_test(resource_name, testfile):
    if(testfile == "1"):
        ...
        with open("tests/regression/regression_output/" + str(resource_name) + "/regression_" + str(resource_name) + "_" + str(testfile) + ".json",  'r') as json_file:
            ...
            assert claim[0]["resource"]["provider"]["type"] == URL[1]
            ...
            assert not claim[0]["request"]["url"] == f'Claim?identifier={CLAIM_IDENTIFIER_SYSTEM}|{MRN}-{VALUE}'
            CLAIM_DATE = claim[0]["resource"]["created"]
            ...
            assert not claim[0]["resource"]["identifier"][0]["value"] == f'{MRN}-{VALUE}', "negative test case failed"
            ...
            assert not claim[0]["resource"]["provider"]["type"] == URL[2], "negative test case failed"
            assert not claim[0]["resource"]["status"] == CLAIM_IDENTIFIER, "negative test case failed"
