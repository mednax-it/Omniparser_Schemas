...
def insurancecoverage_3_test(resource_name, testfile):
    if(testfile == "1"):
        ...
        with open("tests/regression/regression_output/" + str(resource_name) + "/regression_" + str(resource_name) + "_" + str(testfile) + ".json",  'r') as json_file:
            ...
            assert cov[2]["request"]["url"] ==  f'Coverage?identifier=https://pediatrix.com/fhir/NamingSystem/coverage-id|{MRN}-{VALUE}-{CODING_VALUE[2]}', "did not match request url"
            ...
            assert cov[2]["resource"]["beneficiary"]["type"] == BENEFICIARY_TYPE, "did not match beneficiary type"
            ...
            assert cov[2]["resource"]["identifier"][0]["system"] == IDENTIFIER_SYSTEM[0], "did not match identifier system"
            assert cov[2]["resource"]["identifier"][0]["value"] == IDENTIFIER_VALUE[2], "did not match identifier value"
            assert cov[2]["resource"]["identifier"][1]["system"] == IDENTIFIER_SYSTEM[1], "did not match identifier system"
            ...
            assert cov[2]["resource"]["subscriber"]["identifier"]["system"] == SUBSCRIBER_IDENTIFIER_SYSTEM[0]
            ...
            assert cov[2]["resource"]["subscriber"]["identifier"]["value"] == COVERAGE_SUBSCRIBER_VALUE[2], "did not match subscriber identifier value"
            assert cov[2]["resource"]["subscriber"]["reference"] == RELATED_PERSON_INSURANCE_FULL_URL_3 , "did not match subscriber reference"
            assert cov[2]["resource"]["subscriber"]["type"] == COVERAGE_SUBSCRIBER_TYPE[0], "did not match subscriber type"

            #Negative test cases
            assert not cov[2]["fullUrl"] == N_COVERAGE_FULL_URL_1, "negative test case failed"
            assert not cov[2]["request"]["method"] == N_REQUEST_TYPE[0], "negative test case failed"
            assert not cov[2]["request"]["url"] ==  f'Coverage?identifier=https://pediatrix.com/fhir/NamingSystem/coverage-id|{MRN}-{VALUE}-{N_CODING_VALUE[0]}', "negative test case failed"
            assert not cov[2]["resource"]["beneficiary"]["display"] == N_BENEFICIARY_DISPLAY, "negative test case failed"
            assert not cov[2]["resource"]["beneficiary"]["reference"] == N_PATIENT_FULL_URL, "negative test case failed"
            assert not cov[2]["resource"]["beneficiary"]["type"] == N_BENEFICIARY_TYPE, "negative test case failed"
            assert not cov[2]["resource"]["identifier"][1]["type"]["coding"][0]["code"] == N_CODING[0], "negative test case failed"
            assert not cov[2]["resource"]["identifier"][1]["type"]["coding"][0]["display"] == N_CODING[1], "negative test case failed"
            assert not cov[2]["resource"]["identifier"][1]["type"]["coding"][0]["system"] == N_CODING[2], "negative test case failed"
            assert not cov[2]["resource"]["identifier"][1]["value"] == N_CODING_VALUE[0], "negative test case failed"
            assert not cov[2]["resource"]["identifier"][0]["system"] == N_IDENTIFIER_SYSTEM[0], "negative test case failed"
            assert not cov[2]["resource"]["identifier"][0]["value"] == N_IDENTIFIER_VALUE, "negative test case failed"
            assert not cov[2]["resource"]["identifier"][1]["system"] == N_IDENTIFIER_SYSTEM[1], "negative test case failed"
            assert not cov[2]["resource"]["identifier"][1]["value"] == N_CODING_VALUE[0], "negative test case failed"
            assert not cov[2]["resource"]["network"] == N_COVERAGE_NETWORK[0], "negative test case failed"
            assert not cov[2]["resource"]["order"] == N_COVERAGE_ORDER[0], "negative test case failed"
            assert not cov[2]["resource"]["payor"][0]["reference"] == N_ORGANIZATION_PAYOR_FULL_URL, "negative test case failed"
            assert not cov[2]["resource"]["period"]["end"] == N_COVERAGE_PERIOD_END_DATE[0], "negative test case failed"
            assert not cov[2]["resource"]["period"]["start"] == N_COVERAGE_PERIOD_START_DATE[0], "negative test case failed"
            assert not cov[2]["resource"]["policyHolder"]["reference"] == N_PATIENT_FULL_URL, "negative test case failed"
            ...
            assert not cov[2]["resource"]["status"] == N_COVERAGE_STATUS[0], "negative test case failed"
            assert not cov[2]["resource"]["subscriber"]["display"] == N_COVERAGE_SUBSCRIBER_DISPLAY, "negative test case failed"
            assert not cov[2]["resource"]["subscriber"]["identifier"]["value"] == N_COVERAGE_SUBSCRIBER_VALUE, "negative test case failed"
